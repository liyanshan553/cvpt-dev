...
class CVPT(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.VptDrop = nn.Dropout(p=PromptDrop)
        self.Insert = insert
        self.init = init

        if vpt_type == 'Deep':
            self.Prompt_Tokens = nn.Parameter(torch.zeros(depth, Prompt_num, embed_dim))
        else:
            self.Prompt_Tokens = nn.Parameter(torch.zeros(1, Prompt_num, embed_dim))

        if self.init == 1:
            torch.nn.init.xavier_uniform_(self.Prompt_Tokens)
    ...
    def forward_features(self, x):
        ...
        for i in range(len(self.blocks)):
            # get Prompt_Tokens for this block
            Prompt_Tokens = self.VptDrop(self.Prompt_Tokens[i].unsqueeze(0).expand(x.shape[0], -1, -1))
            # lastly remove, a genius trick
            x = self.blocks[i](x, Prompt_Tokens)
...
@torch.no_grad()
def _load_weights(model: CVPT, checkpoint_path: str, prefix: str = ''):
    ...
    for i, block in enumerate(model.blocks.children()):
        ...
        c = model.embed_dim     # embed_dim
        block.CrossAttn.q.weight.copy_(_n2p(w[f'{mha_prefix}query/kernel'].reshape(c, -1)))
        block.CrossAttn.k.weight.copy_(_n2p(w[f'{mha_prefix}key/kernel'].reshape(c, -1)))
        block.CrossAttn.v.weight.copy_(_n2p(w[f'{mha_prefix}value/kernel'].reshape(c, -1)))
        block.CrossAttn.q.bias.copy_(_n2p(w[f'{mha_prefix}query/bias'].reshape(c)))
        block.CrossAttn.k.bias.copy_(_n2p(w[f'{mha_prefix}key/bias'].reshape(c)))
        block.CrossAttn.v.bias.copy_(_n2p(w[f'{mha_prefix}value/bias'].reshape(c)))
        block.CrossAttn.proj.weight.copy_(_n2p(w[f'{mha_prefix}out/kernel']).flatten(1))
        block.CrossAttn.proj.bias.copy_(_n2p(w[f'{mha_prefix}out/bias']))
